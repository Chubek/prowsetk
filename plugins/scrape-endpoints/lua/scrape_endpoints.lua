-- plugins/scrape_endpoints/lua/scrape_endpoints.lua
-- Lua specification layer for the scrape_endpoints plugin.
--
...
local scrape_endpoints = {
    _version = "0.1.0",
    _description = "Scrape endpoints and dump OpenAPI YAML or Postman JSON, optionally resolving chains"
}
...
-- scrape_endpoints-specific flags. Secrets are never echoed.
local function normalize_spec(spec)
    ...
    opts.observe_network = spec.observe_network ~= false
    ...
    opts.spa_probe = spec.spa_probe ~= false
    opts.max_spa_actions = tonumber(spec.max_spa_actions) or 32
...
local function offer_assistant_browser(session, doc, endpoints, opts)
    ...
    io.stderr:write("scrape_endpoints: " .. reason .. ". Launch assistant browser with "
    ...
    io.stderr:write("scrape_endpoints: launching " .. command .. " for " .. url .. "\n")
    os.execute(launch)
    io.stderr:write("scrape_endpoints: press Enter after browser interaction is complete. ")
    io.read("*l")
    return true, handoff
end
...
function scrape_endpoints.render_postman_json(endpoints, spec)
    spec = spec or {}
    ...
    for _, entry in ipairs(ordered) do
        ...
        if ep.request_content_type ~= "application/x-www-form-urlencoded" then
            ...
            for pair in (query .. "&"):gmatch("(.-)&") do
                local key = (pair:match("^([^=]*)") or ""):gsub("%%(%x%x)", function(hex)
                    return string.char(tonumber(hex, 16))
                end)
                names[key] = true
            end
        ...
        if ep.request_content_type and ep.request_content_type ~= "" then
            headers[1] = '{"key":"Content-Type","value":' .. json_quote(ep.request_content_type) .. '}'
        end
        local body = ""
        if ep.request_content_type == "application/x-www-form-urlencoded" then
            local fields = {}
            for _, param in ipairs(ep.parameters or {}) do
                fields[#fields + 1] = '{"key":' .. json_quote(param)
        ...
        items[#items + 1] = '{"name":' .. json_quote(method .. " " .. url)
            .. ',"request":{"method":' .. json_quote(method) .. ',"url":' .. json_quote(url)
            .. ',"description":' .. json_quote(description) .. ',"header":['
            .. table.concat(headers, ",") .. ']' .. body .. '},"response":[]}'
    end
    local description = "Generated by ProwseTk scrape-endpoints. Heuristic API discoveries; "
        .. "not authoritative documentation. Request bodies and credentials are omitted."
    if spec.spa_probe ~= false then
        description = description .. " SPA probing used Flatworm synthetic interactions before export."
    end
    ...
    return '{"info":{"name":' .. json_quote(spec.collection_name or "Discovered API (scrape-endpoints)")
        .. ',"schema":"https://schema.getpostman.com/json/collection/v2.1.0/collection.json",'
        .. '"description":' .. json_quote(description) .. '},"item":['
        .. table.concat(items, ",") .. ']}\n'
end

local function write_postman(path, json)
    if path == nil or path == "" then return false end
    local file = assert(io.open(path, "wb"), "scrape_endpoints: cannot open Postman output")
    local success = file:write(json)
    local closed = file:close()
    assert(success and closed, "scrape_endpoints: cannot write Postman output")
    return true
end
...
function scrape_endpoints.scrape(session_or_document, spec)
    ...
    if doc == nil then
        error("scrape_endpoints.scrape: no document available (provide url/html or load a document first)", 2)
    end
    ...
    if ext ~= nil and ext.endpoints ~= nil and type(ext.endpoints.extract) == "function" then
        ...
        error("scrape_endpoints: lprowsext.endpoints.extract is not available (build without Lua?)", 2)
    end
    ...
    local function write_yaml(path, content)
        ...
        if f == nil then error("scrape_endpoints: cannot open output: " .. tostring(path) .. " (" .. tostring(err) .. ")", 2) end
        f:write(content)
        f:close()
        return true
    end
    ...
    local postman_json = scrape_endpoints.render_postman_json(resolved, opts)
    if opts.output_postman ~= "" then
        write_postman(opts.output_postman, postman_json)
    end
    ...
    local augmented = {
        ...
        write_postman_json = function(_, path) return write_postman(path, postman_json) end
    }
    ...
    setmetatable(augmented, {
        __index = function(t, k)
            ...
            if k == "postman_json" then return function() return postman_json end end
...
function scrape_endpoints.render_openapi_yaml(endpoints, spec)
    ...
    lines[#lines+1] = "  version: '0.1.0'"
    lines[#lines+1] = "  description: >-"
    lines[#lines+1] = "    Generated by ProwseTk scrape_endpoints. Endpoints are"
...
-- Convenience: scrape_endpoints.dump(session, spec) returns yaml string directly.
function scrape_endpoints.dump(session_or_document, spec)
    local r = scrape_endpoints.scrape(session_or_document, spec)
